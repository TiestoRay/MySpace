...
namespace SEPInstance.Tests
{
    public abstract class SEPInstanceTestBase : AbpIntegratedTestBase<SEPInstanceTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected SEPInstanceTestBase()
        {
        ...
        protected void UsingDbContext(Action<SEPInstanceDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SEPInstanceDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SEPInstanceDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SEPInstanceDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SEPInstanceDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SEPInstanceDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SEPInstanceDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SEPInstanceDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SEPInstanceDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SEPInstanceDbContext, Task<T>> func)
        {
            T result;
