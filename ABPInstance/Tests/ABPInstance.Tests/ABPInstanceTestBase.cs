...
namespace ABPInstance.Tests
{
    public abstract class ABPInstanceTestBase : AbpIntegratedTestBase<ABPInstanceTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ABPInstanceTestBase()
        {
        ...
        protected void UsingDbContext(Action<ABPInstanceDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABPInstanceDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABPInstanceDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABPInstanceDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABPInstanceDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPInstanceDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABPInstanceDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPInstanceDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABPInstanceDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABPInstanceDbContext, Task<T>> func)
        {
            T result;
